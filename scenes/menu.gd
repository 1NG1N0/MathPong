class_name Menu2 extends VBoxContainer

signal actioned(item: Control)
@export var pointer: Node
@export var pointer2: Node
var player1_index = 0 
var player2_index = 0 
...
func _unhandled_input(event):
	...
	var items = get_itens()
	if event.is_action_pressed("RIBA"):
		player1_index = max(player1_index - 1, 0)
		update_pointers()
	elif event.is_action_pressed("TABAXOO"):
		player1_index = min(player1_index + 1, items.size() - 1)
		update_pointers() 
	elif event.is_action_pressed("aceitar"):
		actioned.emit(items[player1_index])  # Player 1 confirma ação
